...
minCubes :: [Cubes] -> Cubes
minCubes = foldl minCubes' (Cubes 0 0 0)
  where
    minCubes' (Cubes r1 g1 b1) (Cubes r2 g2 b2) = Cubes (max r1 r2) (max g1 g2) (max b1 b2)

power :: Cubes -> Int
power (Cubes r g b) = r * b * g

main :: IO ()
main = do
  -- content <- readFile "input/demo_02.txt"
  content <- readFile "input/02.txt"
  print $ sum $ powers content
  where
    games = map deserializeGame . lines
    minima = map (minCubes . cubes) . games
    powers = map power . minima
